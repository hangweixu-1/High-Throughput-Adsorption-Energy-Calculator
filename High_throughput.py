        ...
        while True:
            try:
                ...
                if time.time() - t0 > self.timeout:
                    raise TimeoutError("Timeout acquiring lock: %s" % self.lock_path)
...
def build_config(args):
    ...
    assert len(miller) == 3, "Miller index must have exactly 3 components"
    c.TARGET_MILLER = miller

    sc = tuple(int(x) for x in args.supercell.split(","))
    assert len(sc) == 3, "Supercell must have exactly 3 components"
    c.SLAB_SUPERCELL = sc
    ...
    c.USE_TIER_B = True
    c.ADS_CORR = {
        "*H": 0.0, "*CO": 0.0, "*COOH": 0.0, "*CHO": 0.0,
        "*CH2O": 0.0, "*CO2": 0.0, "*OCHO": 0.0, "*": 0.0,
    }
...
def compute_kpts_from_kspacing(atoms: Atoms, kspacing: float, slab_xy_only: bool = True) -> Tuple[int, int, int]:
    """
    Compute k-points from target kspacing in 1/Angstrom.
    Approx: N_i = max(1, ceil(|b_i| / (2*pi * kspacing))) where b_i is reciprocal vector magnitude.
    ...
    if abs(det) < 1e-10:
        return (1, 1, 1)
    recip = 2.0 * math.pi * np.linalg.inv(cell).T  # reciprocal vectors in 1/Angstrom
...
def bottom_n_layers_indices(atoms, n, tol):
    ...
    if len(centers) < n:
        raise ValueError("Only %d layers found, need %d" % (len(centers), n))
    fixed = [i for i, l in enumerate(labels) if l < n]
    return fixed, centers

def ads_key(n):
    return {
        "H": "*H", "CO2": "*CO2", "CO": "*CO", "COOH": "*COOH",
        "CHO": "*CHO", "CH2O": "*CH2O", "OCHO": "*OCHO",
    }.get(n, "*%s" % n)
...
def build_adsorbates() -> Dict[str, Molecule]:
    ...
    a["COOH"] = Molecule(
        ["H", "O", "C", "O"],
        [[0, 0, 0], [0.97, 0, 0], [2.20, 0, 0], [3.36, 0, 0]],
    )
    a["CHO"] = Molecule(["H", "C", "O"], [[0, 0, 0], [1.10, 0, 0], [2.33, 0, 0]])
    a["CH2O"] = Molecule(
        ["H", "H", "C", "O"],
        [[0, 0.9, 0], [0, -0.9, 0], [1.10, 0, 0], [2.33, 0, 0]],
    )

    # OCHO (Formate): bent structure (H-C-O2), not linear.
    a["OCHO"] = Molecule(
        ["H", "C", "O", "O"],
        [
            [0.000, 0.000, 0.000],   # H
...
def build_adsorption_candidates_ase(
    ...
    for i, xyz in enumerate(site_coords_cart):
        ...
        # re-apply constraint (ASE sometimes resets it)
        aa.set_constraint(FixAtoms(indices=fixed_slab_indices))
...
def mace_pre_rank_downselect(
    ...
    """
    MACE mode: cheap pre-ranking (single-point or short relax) -> select topK.
    Returns selected candidates plus a ranking log list.
    """
    if len(candidates) <= max_keep:
...
def process_structure(cfg, cif_file, mace_calc=None):
    ...
    for an in cfg.SCREEN_ADS:
        if an not in ads_mols:
            raise ValueError("Adsorbate '%s' not defined in build_adsorbates()" % an)
    ...
    slabs = generate_all_slabs(
        ...
        min_vacuum_size=cfg.MIN_VACUUM,
        center_slab=True,
    ...
    for ai, aname in enumerate(cfg.SCREEN_ADS, 1):
        ...
        try:
            ...
            # Descriptor value based on corrected energies
...
def main():
    ...
    plog("kspacing      : %.3f 1/Ang" % cfg.KSPACING)
